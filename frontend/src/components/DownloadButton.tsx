...
export default function DownloadButton({ status }: { status: JobStatus }) {
  ...
  const filename = `${status.mod_id || "mod"}-forge-project.zip`;
  ...
  return (
    <div className="text-center space-y-3">
      ...
      <button
        ...
        className="mc-btn w-full py-3 text-[10px] flex items-center justify-center gap-2 text-[#55ff55]"
        style={FONT}
      >
      ...
      <p className="text-[8px] text-[#808080]" style={FONT}>Extract the ZIP and run ./gradlew build to compile your mod</p>
    </div>
  );
}
