...
async def get_gallery(
    ...
    submissions = await get_approved_submissions(sort, category, limit, offset)
    ...
    job_query = (
        supabase.table("jobs")
        .select("id, mod_name, description, created_at, jar_file_url, mod_spec, author_name, model_used")
        .eq("status", "complete")
        .not_.is_("jar_file_url", "null")
...
async def get_my_mods(
    ...
    query = (
        supabase.table("jobs")
        .select("id, mod_name, description, created_at, jar_file_url, mod_spec, author_name, model_used, status")
        .eq("user_id", user_id)
        .order("created_at", desc=True)
        .range(offset, offset + limit - 1)
    )
